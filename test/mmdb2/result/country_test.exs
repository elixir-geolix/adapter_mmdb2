defmodule Geolix.Adapter.MMDB2.Result.CountryTest do
  ...
  test "regular country" do
    ...
    assert 6_252_001 == result.country.geoname_id
    assert "US" == result.country.iso_code
    assert "アメリカ合衆国" == result.country.names[:ja]

    assert 2_635_167 == result.registered_country.geoname_id
  ...
  test "represented country" do
    ...
    assert 6_255_147 == result.continent.geoname_id
    assert "Asie" == result.continent.names[:fr]

    assert 1_694_008 == result.country.geoname_id
    ...
    assert 6_252_001 == result.represented_country.geoname_id
